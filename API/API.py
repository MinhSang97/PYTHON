...
#     user='MiniMDM10',
#     password='MiniMDM10',
#     dsn=cx_Oracle.makedsn('118.69.35.119', '1521', service_name='hhm'),
#     min=1,
#     max=10,
#     increment=1,
#     threaded=True
...
def save_log(token, cong_to, thoi_gian):
    ...
    insert_query = "INSERT INTO Logs (token, cong_to, thoi_gian, thoi_gian_hien_tai) VALUES (:token, :cong_to, TO_DATE(:thoi_gian, 'YYYY-MM-DD HH24:MI:SS'), :thoi_gian_hien_tai)"
...
def get_data():
    ...
    if meter_model == "HHM11-V1":
        query = '''
            SELECT DISTINCT  e.Ma_DIEMDO, e.tenkhachhang, e.METER_ASSET_NO nocongto, e.TV thoigiandoc,
        ...
        query = '''
select z.Ma_DIEMDO, z.tenkhachhang, z.nocongto, y.thoigiandoc, z.dn_huucong_giao, z.dn_huucong_giao_bieu1, z.dn_huucong_giao_bieu2, z.dn_huucong_giao_bieu3,
...
FROM A_Data_catalogue d
inner JOIN BIZ_PUB_DATA_F_ENERGY_D c 
...
SELECT  ZB.CONS_NO, ZB.CONS_NAME, KZ.CON1 AS DXGH, HE.CLDH, BJ.ASSETNO AS nocongto, BJ.METER_MODEL,
CASE
WHEN BJ.DEVICETYPE_ID = 'repeater' THEN TO_CHAR(SJ.MR_TIME_RESTA , 'YYYY-MM-DD HH24:MI:SS')
ELSE
TO_CHAR(SJ.MR_TIME_FA , 'YYYY-MM-DD HH24:MI:SS')
END AS THOIGIANDOC,
CASE
WHEN BJ.DEVICETYPE_ID = 'repeater' THEN
DECODE(SJ.REP_STATUS, '01', 'YES', 'NO')
ELSE
DECODE(SJ.MR_TIME_FA, NULL, 'NO', 'YES')
END AS STATUS
FROM A_EQUIP_METER BJ
                   LEFT JOIN A_MP_EQUIPMENT_RELA re on re.equipmentid=bj.meter_id  AND RE.STATUS = 'Y'
                   LEFT JOIN  A_USAGEPOINT JLD ON JLD.MP_ID = RE.MP_ID
                   LEFT JOIN A_OBJ_EXTEND KZ ON KZ.OBJID = JLD.MP_ID AND KZ.OBJTYPE = '150'
                   LEFT JOIN A_DATA_CATALOGUE ZB ON BJ.METER_ID = ZB.METER_ID and zb.status='1'
                  LEFT JOIN  BIZ_PUB_DATA_OTHER_D SJ on zb.data_id=SJ.data_id and zb.status='1'
                    AND (CASE
                         WHEN BJ.DEVICETYPE_ID = 'repeater' THEN
                          SJ.MR_TIME_RESTA
                         ELSE
                     ...
                     AND (CASE
                       ...
                       END) 
                     <= TO_DATE(thoi_gian)
                          ,A_EQUIP_METER_HES HE
                          ,A_EQUIP_RELATIONSHIP ZDGX
                         WHERE HE.METER_ID = BJ.METER_ID
                           AND BJ.METER_ID=ZDGX.EQUIPMENT_ID 
                           AND ZDGX.STATUS='01'
                           AND CLDH IS NOT NULL
                        
              ) tj group by CONS_NO,CONS_NAME,DXGH,CLDH,nocongto,METER_MODEL,THOIGIANDOC,STATUS
               ORDER BY CLDH ASC)y
on z.nocongto = y.nocongto
        '''
    else:
