...
def save_log(token, cong_to, thoi_gian):
    insert_query = "INSERT INTO Logs (token, cong_to, thoi_gian) VALUES (:token, :cong_to, :thoi_gian)"
    cursor = conn.cursor()
    cursor.execute(insert_query, token=token, cong_to=cong_to, thoi_gian=thoi_gian)
    conn.commit()
    cursor.close()
