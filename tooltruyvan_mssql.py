...
# Hàm xóa kết quả truy vấn
def clear_results():
    result_tree.delete(*result_tree.get_children())  # Xóa tất cả các hàng trong Treeview
    result_text.delete("1.0", tk.END)

        # Xóa các cột trong Treeview
    columns = result_tree["columns"]
    for col in columns:
...
# Tạo button "Clear Results"
clear_button = tk.Button(connection_frame, text="Clear Results", command=clear_results)
