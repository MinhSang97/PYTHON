...
def open_query_library_window():
    ...
    query_library = [
        "SELECT t.fcSaleID,t.fcDcuAddr,t.fcMeterAddr,t.dnngay, t.fifn bieucongto, c.fcMa_Khang madiemdo4 , c.fcTen_Khang tenkh , c.fcDia_Chi FROM (SELECT b.fcSaleID, a.fcDcuAddr, a.fcMeterAddr, a.fcEnergy dnngay, a.fifn FROM FreezeData a INNER JOIN Meter b on a.fcMeterAddr=b.fcMeterAddr and a.ftReadTime>='20230401' and a.ftReadTime<'20230402'  ) t INNER JOIN Meter c on c.fcMeterAddr=t.fcMeterAddr",
        "SELECT * FROM Orders",
    ...
    for query in query_library:
        query_button = tk.Button(query_buttons_frame, text=query, command=lambda q=query: insert_query(q, query_entry))
